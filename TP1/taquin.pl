   ...
   */  
   ...
   %   heuristique1(U, H).  % au debut on utilise l'heuristique 1
      ...
      heuristique1(U, H) :-
         findall(Elem,mal_place(Elem,U),L),
         length(L,H).
...
dm(U,Elem,Dist) :-
    coordonnees(U,Elem,Li,Ci),
    final_state(F),
    coordonnees(F,Elem,Lf,Cf),
    Dist is (abs(Lf-Li)+abs(Cf-Ci)).

loop(_,[],0).
loop(U,[Elem|Next],Sum):-
    loop(U,Next,S),
    dm(U,Elem,Result),
    Sum is S+Result.

heuristique2(U, H) :-
    findall(X,mal_place(X,U),L),
    loop(U,L,H).
...
mal_place(Elem,U):-
   	coordonnees(U,Elem,L,C),
   	final_state(Fin),
   	coordonnees(Fin,P,L,C),
   	Elem\=vide,
   	P\=Elem.
         

all_actions(U,L):-
    findall(Next,rule(_,_,U,Next),L).

all_action_and_state(U,L):-
    findall([M,Next],rule(M,_,U,Next),L).
